...
export const PortfolioCard = memo(function PortfolioCard({
  ...
  // skipped when reduced-motion is requested OR when the card is
  // expanded into articles mode (an 80% panel bouncing would feel
  // silly).
  const cardFloatAnimation = useMemo(
    () =>
      prefersReducedMotion || articlesOpen ? { y: 0 } : { y: [0, -8, 0] },
    [prefersReducedMotion, articlesOpen]
  );
  ...
  const cardFloatTransition = useMemo(
    () => ({
      y: {
        duration: 4,
        repeat: prefersReducedMotion || articlesOpen ? 0 : Infinity,
  ...
  // when hoveredQuadrant flips. Disabled in articles mode since the
  // expanded card occupies the full center.
  const offset = useMemo(() => {
    if (articlesOpen) return { x: "0%", y: "0%" };
  ...
  return (
    ...
    >
      <motion.div
        ...
        className={
          articlesOpen
            ? "bg-black/15 backdrop-blur-md border border-white/10 rounded-2xl shadow-2xl w-[80vw] h-[80vh] max-w-[1400px] max-h-[900px] overflow-hidden"
            : "bg-black/15 backdrop-blur-md border border-white/10 rounded-2xl shadow-2xl w-[600px]"
        }
          ...
          {articlesOpen ? (
            ...
            >
              ...
              >
                ...
                {/* Right cluster — GitHub, LinkedIn, Resume */}
                <motion.div
